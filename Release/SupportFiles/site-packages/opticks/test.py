...
class PcaTestCase(unittest.TestCase):
    ...
    def test_pca(self):
        ...
        self.pass_data_el = \
            opticks.DataElement(subtype_as_void=corr, typ="RasterElement")
...
class MetadataTestCase(unittest.TestCase):
    def setUp(self):
        self.failUnless(load_test_file("ir_bushehr_06jun02_ps.tif", True))
        self.data_el = opticks.DataElement("ir_bushehr_06jun02_ps.tif")
        self.meta = opticks.DynamicObject(self.data_el)
        self.meta.clear()
...
class RasterTestCase(unittest.TestCase):
    ...
    def test_create_raster_element(self):
        ...
        args.columns = 12
        args.bands = 2
        args.interleave = opticks.Interleave.BSQ
        args.encoding = opticks.Encoding.INT1UBYTE
        args.location = opticks.ProcessingLocationPreference.RAM
        args.bad_values = [100, 101, 102]
        self.create_re = opticks.RasterElement("Test element", args)
    ...
    def test_data_accessor_args(self):
        ...
        args.row_start, args.row_end = 10, 11
        args.column_start, args.column_end = 5, 7
        args.band_start, args.band_end = 1, 1
        args.interleave = opticks.Interleave.BSQ
        acc = self.fetch_re.get_data_accessor(args)
        self.failUnless(acc is not None and acc.valid)
    ...
    def test_data_accessor_write(self):
        args = opticks.DataAccessorArgs()
        args.row_start, args.row_end = 0, 1
        args.column_start, args.column_end = 0, 1
        args.interleave = opticks.Interleave.BIP
        acc = self.fetch_re.get_data_accessor(args)
        self.failUnless(acc is not None and acc.valid)
        def do_assign(acc):
            acc[0, 0] = 10
...
class LayerTestCase(unittest.TestCase):
    ...
    def test_displayed(self):
        ...
        glayer.hide()
        glayer.hide() # ensure an exception is not thrown
    ...
    def test_conversion(self):
        ...
        tlayer.convert("PseudocolorLayer")
...
class ViewCreateTestCase(unittest.TestCase):
    ...
    def test_view_create(self):
        self.failUnless(opticks.View("foobar",
                                     element=self.element, create=True))
