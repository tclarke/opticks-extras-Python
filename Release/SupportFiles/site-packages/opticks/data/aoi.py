   ...
   def __init__(self, element = None, raster = None, boundingBox = None):
      ...
      try:
         if raster is not None:
            if element is None:
               self.handle = genwrap("createAoiIteratorOverRaster", AoiIterator, c_void_p, types.DataElement)(element, raster).handle
            else:
               self.handle = genwrap("createAoiIteratorOverRaster", AoiIterator, types.DataElement, types.DataElement)(element, raster).handle
         else:
            if element is None:
               self.handle = genwrap("createAoiIteratorOverBoundingBox", AoiIterator, c_void_p, c_int32, c_int32, c_int32, c_int32)(element, boundingBox[0], boundingBox[1], boundingBox[2], boundingBox[3]).handle
            else:
