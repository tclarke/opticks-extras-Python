...
class DataElement(ctypes.Structure, object):
    ...
    @classmethod
    def all(cls, typ=None):
        """Get all top-level data elements of a specific type. If no type is specified, get all top-level data elements."""
        tempf = _genwrap("getDataElements", ctypes.c_int, ctypes.c_char_p, ctypes.c_int, ctypes.POINTER(DataElement))
        if typ is None:
            typ = ctypes.c_char_p(0)
        cnt = tempf(typ, 0, None)
        elements = (DataElement * cnt)()
        tempf(typ, cnt, elements)
        return [i.leafclass() for i in elements]
...
class WizardNode(ctypes.Structure):
    ...
    def set_value(self, value):
        value = _prep_for_set(value, self.type)
        if isinstance(value, DataVariant):
            self._setWizardNodeValue(self, DataVariant._getDataVariantValue(value))
        elif isinstance(value, ctypes.c_void_p):
...
class Aoi(DataElement):
    ...
    @classmethod
    def all(cls):
        """Get all top-level aoi elements."""
        return DataElement.all("AoiElement")
...
class RasterElement(DataElement):
    ...
    @classmethod
    def all(cls):
        """Get all top-level raster elements."""
        return DataElement.all("RasterElement")
...
class Signature(DataElement):
    ...
    @classmethod
    def all(cls):
        """Get all top-level signature elements."""
        return DataElement.all("Signature")
...
class SignatureSet(DataElement):
    ...
    @classmethod
    def all(cls):
        """Get all top-level signature set elements."""
        return DataElement.all("SignatureSet")
...
class View(ctypes.Structure):
    ...
    @classmethod
    def all(cls, typ="SpatialDataView"):
        """Get all views of a specific type. If no type is specified, get all views."""
        tempf = _genwrap("getViews", ctypes.c_int, ctypes.c_char_p, ctypes.c_int, ctypes.POINTER(View))
        if typ is None:
            typ = ctypes.c_char_p(0)
        cnt = tempf(typ, 0, None)
        views = (View * cnt)()
        tempf(typ, cnt, views)
        return [i for i in views]
...
class GcpList(DataElement):
    ...
    @classmethod
    def all(cls):
        """Get all top-level GCP list elements."""
        return DataElement.all("GcpList")
